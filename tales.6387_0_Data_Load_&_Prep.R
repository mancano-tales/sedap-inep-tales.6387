...
Este é o primeiro arquivo de código do projeto registrado junto ao INEP sob o código #23036.006387/2024-81, usuário: tales.6387. Temos como objetivo estimar e decompor o impacto de políticas como as cotas e o ProUNI na redução das desigualdades de acesso na educação superior no Brasil conforme identificado por Salata et al. (2025) via PNAD. Fazemos isso através de um esforço de replicação e ampliação do desenho de pesquisa apresentado por Senkevicks et al. (2024). A proposta é, tal qual fizeram Senkevicks, Barbosa, Carvalhaes e Ribeiro, realizar um cruzamento das coortes de egressos da educação básica a cada ano de 2008 à 2019. Iniciamos assim, carregando e tratando as bases do Censo Escolar. Através da coluna *CPF_MASC* cruzaremos essa base com a base do ENEM, cujos dados de desempenho acadêmico e dados socioeconômicos, como renda, raça e escolaridade dos pais, são agregados à analise. Por fim, cruzamos essa base resultante com os dados do Censo da Educação Superior, verificando assim quais estudantes que se formaram na educação básica e realizaram o ENEM ingressaram na educação superior. Aqueles que ingressaram na educação superior sem realizarem o ENEM, por hora, serão descartados da análise já que não teremos dados de desempenho nem informações sobre a renda dessas pessoas.

# 1. Configuração Inicial

O Código abaixo será execultado partido de um Rproj com nome tales.6387, o diretório de trabalho é o *("T:/tales.6387/")*. Já os dados estão disponíveis no diretório *("B:/")*.

```{r Verifica o diretório e carrega o pacote tidyverse, include: false}
#| warning: true
#| include: false
